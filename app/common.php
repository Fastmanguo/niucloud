 ...
 * 创建一个公共的下载文件
 */
function create_download_file($name)
{
    $dir = 'download/' . date('Y') . '/' . date('m') . '/' . date('d') . '/';
    if (!is_dir($dir)) {
        dir_mkdir($dir);
    }

    // 替换最后一位.
    $name = preg_replace('/\.(?=[^.]*$)/', '_' . create_no() . '.', $name);
 ...
 * @param int $size_max 单位B
 * @param string $returnType
 * @throws ImagickException
 */
function compress_image(string $image_file, int $size_max = 2048000, string $returnType = 'blob', $number_max = 5)
{
    ...
    while ($image->getImageLength() > $size_max) {

        if ($i > 3) {
            $image->resizeImage(intval($image->getImageWidth() / 2), intval($image->getImageHeight() / 2),
                Imagick::FILTER_LANCZOS, 1);
        } elseif ($image->getImageLength() > $size_max * 2) {
            $image->setImageCompressionQuality(50);
        } elseif ($image->getImageLength() > $size_max * 1.5) {
            $image->setImageCompressionQuality(75);
        } else {
            $image->setImageCompressionQuality(90);
        }
        $image->stripImage();
        $temp = $image->getImageBlob();
        $image->clear();
        $image->readImageBlob($temp);
        $i++;

        if ($i > $number_max) break;

    }

    if ($returnType == 'base64') {
        $res = base64_encode($image->getImageBlob());
    } elseif ($returnType == 'hex') {
        $res = bin2hex($image->getImageBlob());
    } else {
        $res = $image->getImageBlob();
    }
 ...
 * 补全资源文件地址
 */
function fill_resource_url($url)
{
 ...
 * 修改图片尺寸
 */
function resize_image($image_file, $width, $height,$save_path = null, $result_type = 'blob')
{

    $editor = Grafika::createEditor();
    $editor->open($image, $image_file);
    $editor->resizeFit($image, $width, $height);

    if ($save_path){
        $editor->save($image, $save_path, null, null, false, 0777);
        return $save_path;
    }else if ($result_type == 'blob'){
        $temp_file = tmpfile();
        $editor->save($image, $temp_file, null, null, false, 0777);
        return stream_get_contents($temp_file);
    }else if ($result_type == 'base64'){
        $temp_file = tmpfile();
        $editor->save($image, $temp_file, null, null, false, 0777);
        return base64_encode(stream_get_contents($temp_file));
    }

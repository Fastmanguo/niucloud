...
namespace addon\saler_tools\app\service\diy;
...
class DiyService extends BaseAdminService
{
    ...
    const CACHE_TAG = 'SYS_DIY_PAGE_TAG';

    const CACHE_KEY = 'SYS_DIY_PAGE_KEY:';
    ...
    {
        $where[]      = ['site_id', '=', 0];
        $field        = 'id,site_id,title,lang,page_title,name,template,type,mode,is_default,share,visit_count,create_time,update_time,value';
        $order        = "lang desc";
        $search_model = $this->model->where([['site_id', '=', 0]])->withSearch(["title", "type", 'mode', 'addon_name'], $where)->field($field)->order($order)->append(['type_name', 'type_page', 'addon_name']);
    ...
    {
        $order = "update_time desc";
        return $this->model->where([['site_id', '=', 0]])->withSearch(["title", "type", 'mode'], $where)->field($field)->order($order)->select()->toArray();
    }
    ...
    public function getInfo(int $id)
    {
        ...
        return $this->model->field($field)->where([['id', '=', $id], ['site_id', '=', 0]])->findOrEmpty()->toArray();
    }

    public function getInfoByName(string $name)
    {
        $field = 'id,site_id,title,page_title,name,template,type,mode,value,is_default,is_change,share,visit_count';
        return $this->model->field($field)->where([['name', '=', $name], ['site_id', '=', 0], ['is_default', '=', 1]])->findOrEmpty()->toArray();
    }
    ...
    {
        return $this->model->where([['site_id', '=', 0]])->withSearch(['type'], $where)->count();
    }
    ...
    public function edit(int $id, array $data)
    {
        ...
        Cache::tag(app()->appCache->sys('app_tag'))->clear();
        return true;
    }
    ...
    {
        Cache::tag(self::CACHE_TAG)->clear();
        return $this->model->where([['id', '=', $id], ['site_id', '=', 0]])->delete();
    }
    ...
    public function setUse(int $id)
    {
        try {
            ...
            Db::startTrans();
            $this->model->where([['name', '=', $info['name']], ['site_id', '=', 0]])->update(['is_default' => 0]);
            $this->model->where([['id', '=', $id], ['site_id', '=', 0]])->update(['is_default' => 1, 'update_time' => time()]);
            Db::commit();
            return true;
        } catch (Exception $e) {
            Db::rollback();
    ...
    public function getInit(array $params = [])
    {
        ...
        $data['domain_url'] = [
            'wap_domain'     => $this->request->domain() . '/',
            'wap_url'        => $this->request->domain() . '/tools',
            'web_url'        => $this->request->domain() . '/',
            'service_domain' => $this->request->domain() . '/'
        ];
    ...
    {
        $this->model->where([['id', '=', $id], ['site_id', '=', 0]])->update(['share' => $data['share']]);
        return true;
    }
    ...
    {
        $pages = PagesDict::getPages(['type' => $type, 'addon' => $addon, 'site_id' => 0]);
        if (!empty($pages)) {
    ...
    private function setDiyData($params)
    {
        ...
        $addon_index_template = $this->getFirstPageData($addon_flag, $addon, 0);

        $field = 'id,title,page_title,name,template,type,mode,value,is_default,is_change';
        $info  = $this->model->field($field)->where([['name', '=', $addon_flag], ['site_id', '=', 0], ['is_default', '=', 1]])->findOrEmpty()->toArray();
        ...
        if (!empty($addon_index_template)) {
            ...
            $diy_page_list = $this->model->where([
                ['site_id', '=', 0],
                ['type', '=', $params['key']]
            ])->field('id,name,type')->order('update_time desc')->select()->toArray();

            // 多应用时，将首页和个人中心设为系统的
            foreach ($diy_page_list as $k => $v) {
                if ($v['name'] == $params['key']) {
                    $this->model->where([['name', '=', $v['name']], ['site_id', '=', 0]])->update(['is_default' => 0]);
                    $this->model->where([['id', '=', $v['id']], ['site_id', '=', 0]])->update(['is_default' => 1, 'update_time' => time()]);
                    break;
                }
            }
            ...
            if ($params['is_start'] == 1) {
                ...
                if (!empty($other_page)) {
                    ...
                    (new CoreDiyConfigService())->setStartUpPageConfig(0, [
    ...
    public function getDiy($params)
    {
        ...
        return cache_remember($key, function () use ($params) {
            ...
            $diy   = $this->model->field($field)
                ->withSearch(['id', 'name'], $params)
                ->where('site_id', 0)
                ->where('lang', app()->appLang)
                ->findOrEmpty()
                ->toArray();
            ...
            return $diy;
        }, app()->appCache->sys('app_tag'));
    }

    public function modifyLang($data)
    {
        $diy = $this->model->where('site_id', 0)->where('id', $data['id'])->findOrEmpty();
        if ($diy->isEmpty()) throw new AdminException('页面不存在');
        $diy->lang = $data['lang'];
        $diy->save();
        Cache::tag(app()->appCache->sys('app_tag'))->clear();
        return success();
